...
module.exports = {
    ...
    getCaindSeason(cb) {
        ...
        }).then(()=>{return loadClips(['sittingOnTop'])})
        .then(() => {           
            stage = stage.concat(filter(1, 'Insert'))
            stage = stage.concat(filterF(1, 'Slow', [ 'Medium']))
            stage = stage.concat(filterF(2, 'Lassy',  ['Medium','Deep']))
            stage = stage.concat(filterF(1, 'Slow', [ 'Deep']))
            stage = stage.concat(filterF(2, 'Fast',  ['Medium','Deep']))
            stage = stage.concat(filterF(1, 'Slow', [ 'ReallyDeep']))
            stage = stage.concat(filter(1, 'Pullout'))
        }).then(()=>{return loadClips(['backStanding'])})
        .then(() => {           
            stage = stage.concat(filter(1, 'Insert'))
            stage = stage.concat(filterF(1, 'Slow', [ 'Medium']))
            stage = stage.concat(filterF(2, 'Lassy',  ['Medium','Deep']))
            stage = stage.concat(filterF(1, 'Slow', [ 'Deep']))
            stage = stage.concat(filterF(2, 'Fast',  ['Medium','Deep','ReallyDeep']))
            stage = stage.concat(filterF(2, 'Slow', ['Deep','ReallyDeep']))
            stage = stage.concat(filterF(2, 'Fast',  ['Deep','ReallyDeep']))
            stage = stage.concat(filter(1, 'Pullout'))
            
        }).then(()=>{return loadClips(['onBack'])})
        .then(() => {           
            stage = stage.concat(filter(1, 'Insert'))
            stage = stage.concat(filterF(2, 'Lassy',  ['Deep']))
            stage = stage.concat(filterF(2, 'Fast', [ 'Deep']))
            stage = stage.concat(filterF(1, 'Lassy',  ['ReallyDeep']))
            stage = stage.concat(filterF(2, 'Slow', ['Tip', 'Medium']))
            stage = stage.concat(filterF(2, 'Fast',  ['Medium','Deep','ReallyDeep']))
            stage = stage.concat(filter(1, 'Pullout'))
           
           
        }).then(()=>{return loadClips(['onFourStand'])})
        .then(() => {           
            stage = stage.concat(filter(1, 'Insert'))
            stage = stage.concat(filterF(2, 'Lassy',  ['Tip', 'Medium']))
            stage = stage.concat(filterF(1, 'Lassy',  ['Deep','ReallyDeep']))
            stage = stage.concat(filterF(2, 'Slow',  ['Medium','Deep','ReallyDeep']))
            stage = stage.concat(filterF(2, 'Fast',  ['Medium','Deep','ReallyDeep']))
            stage = stage.concat(filter(1, 'Pullout'))
            cb(stage)
        })
