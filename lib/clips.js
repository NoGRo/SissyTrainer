...
module.exports = {
    getByPosition(posBase, cb) {
        var positionsRelated = {
            onFour: [
                'onFour',
                'onFourLiftLeg',
                'onFourStand',
            ...
            onBack: ['onBack'],
            backStanding: ['backStanding'],
            sittingOnTop: ['sittingOnTop']
        }

        db.find({ position: { $in: positionsRelated[posBase] } }, (err, docs) => { cb(docs) })
    },
    getSkip(cb) {
        db.findOne({ state: 'mp4' }, (err, doc) => {
            doc.state = "skip"
